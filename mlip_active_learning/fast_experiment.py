...
BATCH = 16
DEVICE = "cpu"
N_SEEDS = 1   # Single seed mode (use run_seeds.py for multi-seed)
BASE_SEED = int(sys.argv[1]) if len(sys.argv) > 1 else 42
OUTPUT_CSV = f"results/fast_experiment_seed{BASE_SEED}.csv"

torch.manual_seed(BASE_SEED)
np.random.seed(BASE_SEED)

print(f"Seed: {BASE_SEED}, Output: {OUTPUT_CSV}")
